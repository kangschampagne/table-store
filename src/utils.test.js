...
test('sortByComparator', () => {
    ...
    const sorted_case_1 = Utils.sortByComparator(list, (a, b) => {
        // DESC
        return b.id - a.id
    })
    expect(sorted_case_1).toEqual([list[2], list[1], list[0]]);

    const sorted_case_2 = Utils.sortByComparator(list, (a, b) => {
        return Utils.compareStrings(a.name, b.name, 'ASC');
    })
    expect(sorted_case_2).toEqual([list[0], list[2], list[1]]);
})
